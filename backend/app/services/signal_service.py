...
class SignalService:
    def get_congestion_hotspots(self) -> Dict[str, Any]:
        ...
        for hotspot in hotspots.get("hotspots", []):
            midpoint = midpoint_of_edge(
                source=str(hotspot.get("from") or ""),
                destination=str(hotspot.get("to") or ""),
            ) or {}

            enriched_hotspots.append(
                {
                    **hotspot,
                    "traffic_level": hotspot.get("severity"),
    ...
    def get_intersections_status(self) -> Dict[str, Any]:
        ...
        for intersection in intersections:
            ...
            average_waiting_time = float(intersection.get("average_waiting_time", 0))
            ...
            status_records.append(
                {
                    ...
                    "volume_capacity_ratio": round(volume_capacity_ratio, 2),
    ...
    def optimize_signals(
        ...
        intersections = self._build_intersection_data()
        result = optimize_traffic_signals_greedy(
        ...
        for signal in result.get("optimized_signals", []):
            midpoint = midpoint_of_edge(
                source=str(signal.get("from") or ""),
                destination=str(signal.get("to") or ""),
            ) or {}

            optimized_signals.append(
                {
                    **signal,
                    "traffic_level": signal.get("congestion_level"),
                    "latitude": midpoint.get("latitude"),
                    "longitude": midpoint.get("longitude"),
                }
            )
    ...
    def _build_intersection_data(self) -> List[Dict[str, Any]]:
        ...
        for record in traffic_records:
            road_id = str(record.get("road_id") or record.get("id"))
            road = road_lookup.get(road_id, {})

            raw_from_node = road.get("from") or record.get("from")
            raw_to_node = road.get("to") or record.get("to")
            from_node = node_lookup.get(str(raw_from_node), raw_from_node)
            to_node = node_lookup.get(str(raw_to_node), raw_to_node)
            ...
            average_waiting_time = record.get("average_waiting_time") or record.get(
                "waiting_time"
            )
        ...
        return self._fallback_intersections_from_roads(roads)
    ...
    def _build_node_lookup(self) -> Dict[str, str]:
        ...
        for node in neighborhoods + facilities:
            node_id = node.get("id")
    ...
    def _fallback_intersections_from_roads(
        ...
        for road in roads:
            capacity = float(road.get("capacity_vehicles_per_hour", 1000))
            raw_from_node = road.get("from")
            raw_to_node = road.get("to")
            from_node = node_lookup.get(str(raw_from_node), raw_from_node)
            to_node = node_lookup.get(str(raw_to_node), raw_to_node)

            intersections.append(
                {
                    "id": str(road.get("id") or road.get("road_id")),
